...
def free_bacon(score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    tens = get_digit(score, 1)
    ones = get_digit(score, 0)
    baconed_score = (2 * tens) - ones
    if(baconed_score > 1):
        return baconed_score
...
def is_swap(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    player_tens = get_digit(player_score, 1)
    player_ones = get_digit(player_score, 0)
    opponent_tens = get_digit(opponent_score, 1)
    opponent_ones = get_digit(opponent_score, 0)

    player_abs_difference = compute(player_tens, player_ones, abs_difference)
    opponent_abs_difference = compute(opponent_tens, opponent_ones, abs_difference)
    if(player_abs_difference == opponent_abs_difference):
        return True
    return False
    # END PROBLEM 4
...
def make_averaged(fn, num_samples=1000):
    ...
    def averaged(*args):
        ...
        while(count < num_samples):
            total += fn(*args)
            count += 1
        averaged_result = total / num_samples
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    max_average = averaged_roll_dice(num_rolls, dice)
    while(num_rolls <= 10):
        if(averaged_roll_dice(num_rolls, dice) > max_average):
            max_average = averaged_roll_dice(num_rolls, dice)
            max_rolls = num_rolls
        num_rolls += 1
    return max_rolls
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    baconed_score = score + free_bacon_points
    swap = is_swap(baconed_score, opponent_score)
    if(swap):
        if(opponent_score > baconed_score):
            return 0
        else:
            return num_rolls
    elif(free_bacon_points >= margin):
        return 0
    return num_rolls
